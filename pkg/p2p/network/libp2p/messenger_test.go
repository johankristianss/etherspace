...
func TestMessenger(t *testing.T) {
	...
	for {
		...
		err := messenger2.Send(net.Message{From: messenger2.Node, To: messenger1.Node, Payload: []byte("Hello")}, ctx)
		cancel()
		if err != nil {
	...
	ctx, cancel := context.WithTimeout(context.Background(), 1000*time.Millisecond)
	err = messenger2.Send(net.Message{From: messenger2.Node, To: messenger1.Node, Payload: []byte("Hello 2")}, ctx)
	cancel()
	assert.Equal(t, err, nil)
